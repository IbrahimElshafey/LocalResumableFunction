Useless because I will use MassTransit to call the service and use the `Request-Response` pattern to get the result. MassTransit will handle the retry policy and the failure recovery.
# توصيف المشكلة
